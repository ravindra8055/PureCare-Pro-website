...
const AIAssistant: React.FC = () => {
    ...
    return (
        <div className="fixed bottom-6 right-6 z-[100] flex flex-col items-end gap-3 font-sans print:hidden">

            {/* Expanded Slots (Mobile: Horizontal / Desktop: Vertical) */}
            <div className={`flex flex-col md:flex-col gap-3 transition-all duration-500 origin-bottom ${isExpanded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0 pointer-events-none'}`}>
            ...
            <button
                ...
                className={`w-16 h-16 rounded-full shadow-2xl flex items-center justify-center text-white transition-all duration-500 hover:scale-110 active:scale-95 z-10 ${isExpanded ? 'bg-gray-900 rotate-180' : 'bg-blue-600'}`}
            >
                {isExpanded ? (
