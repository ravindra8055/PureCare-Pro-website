...
import { COMPANY_NAME, PRIMARY_CITY, PHONE_NUMBER, ADDRESS_OBJ } from '../constants';
...
export const generateMeta = (service: ServiceInfo, area?: AreaInfo): SEOData => {
  ...
  return {
    ...
    canonical: area 
      ? `https://purecarepro.com/${service.slug}-in-${area.slug}-${city.toLowerCase()}`
      : `https://purecarepro.com/services/${service.slug}`,
  };
};
